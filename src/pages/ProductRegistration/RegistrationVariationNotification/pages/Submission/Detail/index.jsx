...
import { ArrowLeft, Check, History, Layers, ListChecks, Plus, SendHorizontal } from "lucide-react";
import { useLocation, useNavigate } from "react-router-dom";

/** Hooks */
import authStore from "@/stores/authStore";
import { useGetProjectTaskHistory, useGetSubmissionDetail, usePatchConfirmAndSubmitSubmission } from "../../../hooks/useSubmission";
...
import RegistrationVersion from "./components/RegistrationVersion";
import SubmissionDetailHeader from "./components/SubmissionDetailHeader/index";
...
import {
  projectTaskCodeByName,
  projectTaskType,
  requestStatusVariantMap,
  submissionStatus,
  taskStatus
} from "../../../constants/general";
import { usePatchAddProjectTaskVersion } from "../../../hooks/useTask";
...
const SubmissionDetail = () => {
  ...
  const { submission, isLoadingSubmissionDetail, refetchSubmissionDetail } = useGetSubmissionDetail(location?.state?.submissionId);
  ...
  if (!location?.state?.submissionId) return navigate("/product-registration");

  return (
    <Skeleton className="w-full h-full" isLoading={isLoadingSubmissionDetail || isPendingPatchAddProjectTaskVersion || isLoadingConfirmAndSubmitSubmission}>
      <div className="p-4">
        ...
        <Accordion
          ...
          className="mb-4"
        >
          <RegistrationVersion marketingAuthorizations={submission?.REGISTRATION_VERSIONS} />
        </Accordion>
        <Accordion
          defaultOpen
          title="Project Task"
