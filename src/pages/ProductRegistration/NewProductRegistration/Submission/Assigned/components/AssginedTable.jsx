...
import { newRegSubmissionAssignedColumns } from "../../../constants/tables";
import { useGetSubmissions } from "../../../hooks/useNewRegSubmission";

const AssignedTable = ({ statusKey }) => {
  const navigate = useNavigate();

  const { submissions: data, refetchSubmissions } = useGetSubmissions(statusKey);

  useEffect(() => {
    refetchSubmissions();
  }, [refetchSubmissions]);

  const columnsWithFilter = newRegSubmissionAssignedColumns.map((column) => {
